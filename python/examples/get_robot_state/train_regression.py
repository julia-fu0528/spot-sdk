...
def train(X_train, X_val, y_train, y_val, model_path, best_model_path, log_dir, ground_truth_coords):
    # Create the model
    ...
    def named_regression_accuracy(y_true, y_pred):
        return euclidean_distance(y_true, y_pred, ground_truth_coords)
...
def regression_accuracy(y_true, y_pred, ground_truth_coords):
    ...
    ground_truth_expanded = tf.expand_dims(ground_truth_coords, axis=0)  # Shape: [1, num_classes, 3]

    # Compute pairwise Euclidean distances
    distances = tf.sqrt(tf.reduce_sum(tf.square(y_pred_expanded - ground_truth_expanded), axis=-1))  # Shape: [batch_size, num_classes]
    ...
    # Check if the nearest ground truth coordinate equals y_true
...
if __name__ == "__main__":
    ...
    model, history = train(X_train, X_val, y_train, y_val, model_path, best_model_path, log_dir, coordinates)
