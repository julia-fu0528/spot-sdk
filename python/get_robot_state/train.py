...
def main(num_classes, markers_path, classify, seq):
    ...
    logs_dir = os.path.join(Path(__file__).parent.parent, "gouger_logs")

    if classify:
        tb_logger = TensorBoardLogger(logs_dir, name="classification")
    else:
        tb_logger = TensorBoardLogger(logs_dir, name="regression")
    ...
    checkpoint_callback = ModelCheckpoint(dirpath = Path(__file__).parent.parent, monitor="val_acc", mode="max", save_last=True, filename="best")
    early_stop_callback = EarlyStopping(dirpath = Path(__file__).parent.parent, monitor="val_acc", patience=100, mode="max")
